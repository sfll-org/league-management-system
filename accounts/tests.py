...
class LoginViewTests(TestCase):
    ...
    def test_safe_next_url_followed(self):
        resp = self.client.post(
            reverse("accounts:login") + "?next=/players/",
            {"email": "login@sfll.org", "password": "testpass123"},
        )
        self.assertRedirects(resp, "/players/", fetch_redirect_response=False)
    ...
    def test_external_next_url_rejected(self):
        ...
        self.assertNotEqual(resp["Location"], "https://evil.example/phish")
        self.assertEqual(resp["Location"], "/dashboard/")

    def test_protocol_relative_next_url_rejected(self):
        resp = self.client.post(
            reverse("accounts:login") + "?next=//evil.example/phish",
            {"email": "login@sfll.org", "password": "testpass123"},
        )
        self.assertNotEqual(resp["Location"], "//evil.example/phish")
        self.assertEqual(resp["Location"], "/dashboard/")

    def test_no_next_uses_login_redirect_url(self):
        resp = self.client.post(
            reverse("accounts:login"),
            {"email": "login@sfll.org", "password": "testpass123"},
        )
