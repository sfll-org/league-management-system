...
def _get_user_roles(user):
    ...
    if user.is_superuser:
        return {'cto'}
    return set(UserRole.objects.filter(user=user, is_active=True).values_list('role', flat=True))
...
def dashboard(request):
    ...
    if _is_admin(roles):
        ...
        # RSVP rates
        total_assignments = SessionAssignment.objects.filter(session__season=active_season).count()
        total_rsvps = RSVP.objects.filter(session__season=active_season).count()
        context['rsvp_rate'] = round((total_rsvps / total_assignments * 100) if total_assignments > 0 else 0)

        # Check-in rates
        total_checkins = CheckIn.objects.filter(session_assignment__session__season=active_season).count()
        context['checkin_rate'] = round((total_checkins / total_assignments * 100) if total_assignments > 0 else 0)
        ...
        draft_sessions = DraftSession.objects.filter(season=active_season).select_related('division')
    ...
    if _is_coach(roles):
        try:
            ...
            coach_season = CoachSeason.objects.filter(
                coach=coach, season=active_season
            ).select_related('team_season', 'team_season__team', 'team_season__division').first()
    ...
    if _is_front_desk(roles):
        today = timezone.now().date()
        context['todays_sessions'] = sessions.filter(date=today).select_related('division')
