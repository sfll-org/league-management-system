...
def cto_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        if not _require_cto(request.user):
            messages.error(request, 'You do not have permission to access the import system.')
...
def import_upload(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'core/imports/results.html', {
            'import_run': import_run,
            'stats': importer.stats,
        })

    return render(request, 'core/imports/upload.html', {
        'seasons': seasons,
    })
...
def import_review(request, run_id):
    ...
    flags = import_run.flags.select_related('player_season__player').order_by('-created_at')
    ...
    return render(request, 'core/imports/review.html', {
        'import_run': import_run,
        'flags': flags,
