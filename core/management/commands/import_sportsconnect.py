...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "csv_file", type=str, help="Path to the SportsConnect CSV file."
        )
        parser.add_argument(
            "--season",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if season_name:
            try:
                ...
                raise CommandError(
                    f'Season "{season_name}" not found for league {league_name}.'
                )
        else:
            season = Season.objects.filter(league=league, is_active=True).first()
            if not season:
                raise CommandError(
                    "No active season found. Use --season to specify one."
                )
    ...
    def _print_summary(self, import_run, importer):
        ...
        if stats["flagged_for_review"] > 0:
            ...
            if import_run.pk:
                self.stdout.write(
                    f"  Review at: /admin/imports/{import_run.pk}/review/"
                )
