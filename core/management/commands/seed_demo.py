...
FIRST_NAMES = [
    'Jayden', 'Aiden', 'Liam', 'Noah', 'Ethan', 'Mason', 'Lucas', 'Oliver',
    'Elijah', 'Logan', 'Sophia', 'Isabella', 'Mia', 'Charlotte', 'Amelia',
    'Harper', 'Evelyn', 'Abigail', 'Emily', 'Luna',
]

LAST_NAMES = [
    'Rodriguez', 'Martinez', 'Garcia', 'Lopez', 'Hernandez', 'Gonzalez',
    'Perez', 'Sanchez', 'Ramirez', 'Torres', 'Rivera', 'Gomez', 'Diaz',
    'Reyes', 'Morales', 'Cruz', 'Ortiz', 'Gutierrez', 'Chavez', 'Ramos',
]

TEAM_NAMES = [
    'Marlins', 'Panthers', 'Dolphins', 'Hurricanes', 'Gators',
    'Rays', 'Thunder', 'Lightning', 'Heat', 'Sharks',
    'Eagles', 'Tigers',
]

HITTING_FIELDS = [
    {'key': 'swing_mechanics', 'label': 'Swing Mechanics', 'type': 'rating', 'min': 1, 'max': 5},
...
OUTFIELD_FIELDS = [
    ...
    {'key': 'arm_strength', 'label': 'Arm Strength', 'type': 'rating', 'min': 1, 'max': 5},
    {'key': 'speed', 'label': 'Speed', 'type': 'rating', 'min': 1, 'max': 5},
]

PITCHING_FIELDS = [
    {'key': 'mechanics', 'label': 'Pitching Mechanics', 'type': 'rating', 'min': 1, 'max': 5},
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            admin.save()
            self.stdout.write(self.style.SUCCESS('  Created superuser: admin@sfll.org / admin123'))
        else:
            ...
            if div.has_leagues:
                ...
                for sub in div.league_names:
                    for _ in range(2):
                        ...
                        team, _ = Team.objects.get_or_create(league=league, name=f'{div_name} {t_name}')
        ...
        for div_name, ts_list in team_seasons.items():
            for ts in ts_list:
                ...
                coach_obj, _ = Coach.objects.get_or_create(
                    user=hc_user, league=league,
                )
                CoachSeason.objects.get_or_create(
                    coach=coach_obj, team_season=ts,
                    defaults={'season': season, 'role': 'head_coach', 'is_drafter': True},
                )
                coach_count += 1
                ...
                if random.random() > 0.5:
                    ...
                    ac_coach, _ = Coach.objects.get_or_create(
                        user=ac_user, league=league,
                    )
                    CoachSeason.objects.get_or_create(
                        coach=ac_coach, team_season=ts,
            ...
            for _ in range(num_players):
                ...
                player, _ = Player.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'first_name': first,
                        'last_name': last,
                        'date_of_birth': date(2026, 1, 1) - timedelta(days=random.randint(3000, 4500)),
                    },
                )
        ...
        # 10. Email Templates
        templates_data = [
            ('SES Invite', 'You are invited to {{ session_name }}',
    ...
    def _flush(self):
        ...
        from communications.models import EmailLog, RSVP
