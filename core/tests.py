...
class CmdkUrlEncodingTests(TestCase):
    ...
    def setUp(self):
        from core.views import _cmdk_families, _cmdk_players
...
class CmdkPageRoleFilterTests(TestCase):
    ...
    def _user_with_roles(self, email, roles=()):
        ...
        for role in roles:
            UserRole.objects.create(
                user=user, role=role, league=self.league, is_active=True
            )
        return user
...
class ImportTriggerViewTests(TestCase):
    ...
    def test_redirects_with_error_when_multiple_leagues(self):
        League.objects.create(
            name="League A",
            short_name="LA",
            sportsconnect_report_url="https://a.example.com",
        )
        League.objects.create(
            name="League B",
            short_name="LB",
            sportsconnect_report_url="https://b.example.com",
        )
        resp = self.client.post(reverse("import_trigger"))
        self.assertRedirects(resp, reverse("import_history"))
        msgs = list(resp.wsgi_request._messages)
