...
from .models import Division, Player, PlayerSeason, Season, TeamSeason
...
def index(request):
    ...
    # ⌘K palette deep-links here with ?family= so the palette's family
    # choices land on the filtered roster instead of the full list.
    search_family = request.GET.get('family', '').strip()
    if search_family:
        qs = qs.filter(account_name=search_family)
