...
from players.models import Division, League, Player, PlayerSeason, Season, Station, Team, TeamSeason
...
def _setup_eval_base():
    ...
    station = Station.objects.create(
        league=league, name='Hitting', display_order=0,
        eval_fields=[
    ...
    team_season = TeamSeason.objects.create(
        team=team, season=season, division=division,
    )
    # Coach
    ...
    player = Player.objects.create(
        league=league, sportsconnect_player_id='SC-001',
        first_name='Jayden', last_name='Rodriguez',
    )
    ps = PlayerSeason.objects.create(
        player=player, season=season, division=division,
    )

    # Session + checked-in assignment
    session = Session.objects.create(
        season=season, name='SES 1', date=date(2026, 3, 28),
        start_time=time(9, 0), division=division,
    )
    assignment = SessionAssignment.objects.create(
        session=session, player_season=ps, assigned_by=coach_user,
    )
    ...
    return {
        ...
        'station': station, 'team': team, 'team_season': team_season,
        'coach_user': coach_user, 'coach': coach, 'coach_season': coach_season,
        'player': player, 'ps': ps, 'session': session, 'assignment': assignment,
    }
...
class CoachRankingModelTests(TestCase):
    ...
    def test_ordering(self):
        p2 = Player.objects.create(
            league=self.d['league'], sportsconnect_player_id='SC-002',
            first_name='Aiden', last_name='Martinez',
        )
        ps2 = PlayerSeason.objects.create(
            player=p2, season=self.d['season'], division=self.d['division'],
        )
        CoachRanking.objects.create(
            coach_season=self.d['coach_season'], player_season=ps2, rank_order=1,
        )
        CoachRanking.objects.create(
            coach_season=self.d['coach_season'], player_season=self.d['ps'], rank_order=2,
        )
        rankings = list(CoachRanking.objects.filter(coach_season=self.d['coach_season']))
...
class EvalViewPermissionTests(TestCase):
    ...
    def test_eval_player_requires_coach_season(self):
        ...
        user = User.objects.create_user(
            username='staffonly@sfll.org', email='staffonly@sfll.org',
            first_name='Staff', last_name='Only', password='testpass123',
            is_staff=True,
        )
...
class SaveEvalTests(TestCase):
    ...
    def test_save_eval_upsert(self):
        ...
        self.client.post(url, {'score_power': '5', 'score_contact': '5', 'action': 'save'})
        self.client.post(url, {'score_power': '9', 'score_contact': '8', 'action': 'save'})
        self.assertEqual(Evaluation.objects.filter(
            coach_season=self.d['coach_season'],
            station=self.d['station'],
            player_season=self.d['ps'],
        ).count(), 1)
...
class EvalPrivacyTests(TestCase):
    ...
    def setUp(self):
        ...
        self.coach2_user = User.objects.create_user(
            username='coach2@sfll.org', email='coach2@sfll.org',
        ...
        ts2 = TeamSeason.objects.create(
            team=team2, season=self.d['season'], division=self.d['division'],
        )
        self.coach2_season = CoachSeason.objects.create(
            coach=coach2, team_season=ts2, season=self.d['season'], role='head_coach',
        )
...
class DivisionReportPermissionTests(TestCase):
    ...
    def test_division_report_forbidden_for_coach(self):
        ...
        user = User.objects.create_user(
            username='justcoach@sfll.org', email='justcoach@sfll.org',
            first_name='Just', last_name='Coach', password='testpass123',
        )
        UserRole.objects.create(
            user=user, league=self.d['league'], role='head_coach',
            division=self.d['division'], is_active=True,
        )
        self.client.login(username='justcoach@sfll.org', password='testpass123')
        resp = self.client.get(reverse(
            'evaluations:division_report', args=[self.d['division'].pk],
        ))
        self.assertEqual(resp.status_code, 403)

    def test_division_report_allowed_for_player_agent(self):
        user = User.objects.create_user(
            username='pa@sfll.org', email='pa@sfll.org',
            first_name='Player', last_name='Agent', password='testpass123',
        )
        UserRole.objects.create(
            user=user, league=self.d['league'], role='player_agent',
            division=self.d['division'], is_active=True,
        )
    ...
    def test_player_agent_wrong_division_denied(self):
        ...
        user = User.objects.create_user(
            username='pa-aaa@sfll.org', email='pa-aaa@sfll.org',
            first_name='PA', last_name='AAA', password='testpass123',
        )
        UserRole.objects.create(
            user=user, league=self.d['league'], role='player_agent',
            division=other_div, is_active=True,
        )
