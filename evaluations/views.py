...
def _get_coach_season(user, session):
    ...
    try:
        ...
        return (
            CoachSeason.objects.select_related("team_season")
            .filter(
                coach__user=user,
                season=session.season,
            )
...
def _get_coach_season_for_season(user, season):
    ...
    try:
        ...
        return (
            CoachSeason.objects.select_related("team_season")
            .filter(coach__user=user, season=season)
            .first()
        )
...
def station_session_eval(request, station_id, session_id):
    ...
    session = get_object_or_404(
        Session.objects.select_related("season", "division"), pk=session_id
    )

    # Prevent cross-league station/session enumeration
    if station.league_id != session.season.league_id:
        return HttpResponseForbidden(
            "Station does not belong to this session's league."
        )
    ...
    if not _is_global_eval_role(request.user):
        if (
            not coach_season
            or session.division_id != coach_season.team_season.division_id
        ):
            return HttpResponseForbidden(
...
def eval_player(request, station_id, session_id, player_season_id):
    ...
    if player_season.season_id != session.season_id:
        return HttpResponseForbidden(
            "Player is not registered for this session's season."
        )
    ...
    if not _is_global_eval_role(request.user):
        ...
        if player_season.division_id != coach_season.team_season.division_id:
            return HttpResponseForbidden(
                "You may only evaluate players in your own division."
            )
...
def player_eval_view(request, player_season_id):
    ...
    if not _is_global_eval_role(request.user):
        if player_season.division_id != coach_season.team_season.division_id:
            return HttpResponseForbidden(
                "You may only view players in your own division."
            )
