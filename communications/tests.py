...
from communications.models import EmailLog, EmailTemplate, RSVP
...
def _setup_comms_base():
    ...
    ps = PlayerSeason.objects.create(
        ...
        account_name='Maria Rodriguez', account_email='maria@example.com',
    )
    session = Session.objects.create(
        season=season, name='SES 1', date=date(2026, 3, 28),
        start_time=time(9, 0), division=division, location='Field 3',
    )
    ...
    return {
        ...
        'player': player, 'ps': ps, 'session': session,
...
def _create_comms_user(email='comms@sfll.org', password='testpass123'):
    return User.objects.create_user(
        ...
        first_name='Comms', last_name='Editor', password=password,
    )
...
class EmailTemplateModelTests(TestCase):
    ...
    def test_default_active(self):
        t = EmailTemplate.objects.create(
            league=self.d['league'], name='Test', subject_template='s', body_template='b',
        )
...
class EmailLogModelTests(TestCase):
    ...
    def test_ordering_by_sent_at(self):
        EmailLog.objects.create(
            player_season=self.d['ps'], to_address='a@b.com',
            subject='First', body_snapshot='body',
        )
        EmailLog.objects.create(
            player_season=self.d['ps'], to_address='a@b.com',
            subject='Second', body_snapshot='body',
        )
...
class CommsViewPermissionTests(TestCase):
    ...
    def test_comms_home_forbidden_for_regular_user(self):
        user = _create_comms_user(email='nobody@sfll.org')
    ...
    def test_comms_home_allowed_for_comms_editor(self):
        ...
        UserRole.objects.create(
            user=user, league=self.d['league'], role='comms_editor', is_active=True,
        )
    ...
    def test_comms_home_allowed_for_superuser(self):
        user = User.objects.create_user(
            ...
            first_name='S', last_name='U', password='testpass123',
            is_superuser=True,
        )
    ...
    def test_template_list_requires_comms_role(self):
        user = _create_comms_user(email='nobody2@sfll.org')
...
class PublicRSVPTests(TestCase):
    ...
    def test_rsvp_submit_attending(self):
        ...
        rsvp = RSVP.objects.get(
            player_season=self.d['ps'], session=self.d['session'],
        )
    ...
    def test_rsvp_invalid_status(self):
        ...
        self.assertFalse(
            RSVP.objects.filter(player_season=self.d['ps']).exists()
        )

    def test_rsvp_no_session_assignment(self):
        """Player with no session assignment should see error."""
        player2 = Player.objects.create(
            league=self.d['league'], sportsconnect_player_id='SC-999',
            first_name='No', last_name='Session',
        )
        ps2 = PlayerSeason.objects.create(
            player=player2, season=self.d['season'], division=self.d['division'],
        )
        resp = self.client.get(
            reverse('public_rsvp', args=[ps2.rsvp_token])
        )
        self.assertEqual(resp.status_code, 200)
