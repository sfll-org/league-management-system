...
def draft_home(request):
    ...
    if request.user.is_superuser:
        ...
        if roles.filter(role__in=["cto", "ses_manager"]).exists():
            ...
            pa_division_ids = roles.filter(role="player_agent").values_list(
                "division_id", flat=True
            )
            if pa_division_ids:
                seeding_divisions = list(
                    Division.objects.filter(
                        pk__in=pa_division_ids,
                        is_active=True,
                    )
                )
...
def coach_rankings(request):
    ...
    player_seasons = (
        ...
        .select_related("player", "coaches_child_of__coach__user")
        .order_by(
            "player__last_name",
            "player__first_name",
        )
...
def save_rankings(request):
    ...
    if not coach_season or coach_season.role != "head_coach":
        return JsonResponse(
            {"error": "Only head coaches can submit rankings."}, status=403
        )
...
def seeding(request, division_id):
    ...
    players_data.sort(
        key=lambda p: (p["overall_avg"] is None, -(p["overall_avg"] or 0))
    )
...
def draft_board(request, session_id):
    ...
    return render(
        ...
        "draft/draft_board.html",
        {
            ...
            "available_json": json.dumps(available_data),
            "is_admin": is_admin,
...
def undo_pick(request, session_id):
    ...
    if num_teams > 0:
        draft_session.current_round = (
            (draft_session.current_pick - 1) // num_teams
        ) + 1
...
def complete_draft(request, session_id):
    ...
    if not _can_manage_draft(request.user, division):
        return HttpResponseForbidden(
            "You do not have permission to complete this draft."
        )
