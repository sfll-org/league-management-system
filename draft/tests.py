...
class DraftRoomViewTests(TestCase):
    ...
    def test_draft_room_excludes_drafted_players(self):
        ...
        picks_block = picks_match.group(1) if picks_match else ""
        available_block = available_match.group(1) if available_match else ""
        picks_data = _json.loads(picks_block) if picks_block else []
        available_data = _json.loads(available_block) if available_block else []
